...
def sub_loop_1s() :
    ...
    for order_id in bt.order_details :
        ...
        if status == 'Completed' :
            ...
            if HOLD > MIN_HOLD :
                if HOLD <= AMOUNT*2 :
                    print_log('HOLD(='+str(HOLD)+') <= 2 AMOUNT(='+str(AMOUNT*2)+')')
            ...
            if HOLD < MAX_HOLD :
                if HOLD >= AMOUNT*18 :
                    print_log('HOLD(='+str(HOLD)+') >= 18 AMOUNT(='+str(AMOUNT*18)+')')
...
def get_HOLD() :
    return bt.hold_position

def get_TOTAL_BALANCE() :
    return int((bt.hold_position)*bt.market_price + bt.krw)
